...
export const EventItem: React.FC<{ eventData: Event }> = ({ eventData }) => {
  ...
  const channelName =
    channels.find((channel) => channel.id === eventData.channelId)?.name || "Unknown Channel";
  return (
    <>
      <div
        ...
        onClick={() => {
          if (selectedProject) {
            navigate(
              `/dashboard/projects/${selectedProject.id}/channels/${eventData.channelId}/events/${eventData.id}`
            );
          }
        }}
      >
        ...
        <div className="flex min-w-0 flex-1 justify-between space-x-4">
          <div>
            <p className="text-md text-white ">{eventData.title}</p>
            <p className=" text-md text-gray-400">
              <span>{channelName}</span> <span className="text-gray-500 mx-2">•</span>{" "}
              <span>{formattedDate}</span>
            </p>

            {showInlineDetails && (
              <>
                <p className="text-lg text-gray-400">{eventData.description}</p>
                <p className="mt-4">
                  {eventData.tags &&
                    Object.entries(eventData.tags)
                      .slice(0, 1)
                      .map(([key, value]) => (
                        <span
                          key={key}
                          className="mr-2 mb-2 inline-flex items-center rounded-md
